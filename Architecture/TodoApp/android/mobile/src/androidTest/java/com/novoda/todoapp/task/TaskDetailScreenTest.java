...
public class TaskDetailScreenTest {
     ...
     */
    private static Task ACTIVE_TASK = Task.builder()
            .id(Id.from("1"))
            .title(TASK_TITLE)
     ...
     * {@link Task} stub that is added to the fake service API layer.
     */
    private static Task COMPLETED_TASK = Task.builder()
            .id(Id.from("2"))
            .title(TASK_TITLE)
            .description(TASK_DESCRIPTION)
            .isCompleted(true)
    ...
    private void startActivityWithWithStubbedTask(Task task) {
        ...
        TodoApplication.TASKS_SERVICE.save(task);

        // Lazily start the Activity from the ActivityTestRule this time to inject the start Intent
        Intent startIntent = new Intent();
        startIntent.putExtra(AndroidNavigator.EXTRA_TASK_ID, task.id().value());
    ...
    public void orientationChange_menuAndTaskPersist() {
        ...
        onView(withId(R.id.delete_task_button)).check(matches(isDisplayed()));
