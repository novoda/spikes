...
public class RxValueListener<T> implements ValueEventListener {
    ...
    public static <T> Observable<T> listenToValueEvents(final DatabaseReference databaseReference, final Func1<DataSnapshot, T> marshaller) {
        return Observable.create(new Observable.OnSubscribe<T>() {
            ...
            public void call(Subscriber<? super T> subscriber) {
                final ValueEventListener eventListener = databaseReference.addValueEventListener(new RxValueListener<T>(subscriber, marshaller));
