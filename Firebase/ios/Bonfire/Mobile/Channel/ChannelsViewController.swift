...
final class ChannelsViewController: UIViewController {
    ...
    static func withDependencies() -> ChannelsViewController {
        return ChannelsViewController(channelsService: SharedServices.channelsService)
    }

    init(channelsService: ChannelsService) {
        self.channelsPresenter = ChannelsPresenter(channelsService: channelsService, channelsDisplayer: channelsView)
