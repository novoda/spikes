...
class ModelVisualiserRajawali(private val renderer: NovodaARCoreRajawaliRenderer,
    ...
    private fun createTouchAnchors(camera: Camera, frame: Frame) {
        tapHelper.poll()?.apply {
            if (camera.trackingState == TrackingState.TRACKING) {
                debugViewDisplayer.append("Tap while tracking")
                for (hit in frame.hitTest(this)) {
                    // Check if any plane was hit, and if it was hit inside the plane polygon
                    val trackable = hit.trackable


                    debugViewDisplayer.append("There's a hit")
                    // Creates an anchor if a plane or an oriented point was hit.
                    if (trackable is Plane && trackable.isPoseInPolygon(hit.hitPose) ||
                            trackable is Point && trackable.orientationMode == Point.OrientationMode.ESTIMATED_SURFACE_NORMAL) {

                        debugViewDisplayer.append("\t Tapped on plane. Creating anchor...")
